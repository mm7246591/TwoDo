...
description: Vue.js 任務必須使用。強烈建議以 Composition API 搭配 `<script setup>` 與 TypeScript 作為標準做法。涵蓋 Vue 3、SSR、Volar、vue-tsc。任何 Vue、.vue 檔案、Vue Router、Pinia 或 Vite with Vue 工作都要載入。除非專案明確要求 Options API，否則一律使用 Composition API。
license: MIT
metadata:
