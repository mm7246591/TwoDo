...
const shouldShowSkipToHome = computed(
  () => hasPartner.value || isFirstPairingEntry.value,
);
...
<template>
  <MobileAppShell>
    <section
      class="app-page-content relative flex flex-1 items-center justify-center overflow-hidden py-[var(--app-space-24)] font-['Plus_Jakarta_Sans'] text-on-background">
      <div class="pointer-events-none absolute inset-[0px] z-0">
        <div class="absolute inset-[0px] bg-gradient-to-t from-background via-background/95 to-background/60" />
      </div>
      ...
      <section class="relative z-10 w-full max-w-[420px]">
        <div
          ...
          <section class="group relative mb-md overflow-hidden rounded-2xl bg-surface-container p-md">
            <div
              ...
              aria-hidden="true" />
            <p class="relative z-10 mb-sm font-label-sm text-label-sm text-on-surface-variant">
              你的邀請碼
            </p>
            <div class="relative z-10 flex items-center justify-between gap-md">
              <div class="font-headline-md text-headline-md font-bold tracking-[0.2em] text-on-surface">
                {{ myInviteCode || "--------" }}
              </div>
              <button aria-label="複製邀請碼"
                ...
                type="button" :disabled="!hasInviteCode || isCopying" @click="handleCopyInviteCode">
          ...
          <div class="my-md flex items-center gap-md">
            ...
            <span class="font-label-sm text-label-sm uppercase tracking-widest text-outline">或</span>
            <div class="h-px flex-1 bg-outline-variant" />
          </div>

          <section class="mb-lg">
            <label class="mb-sm block font-label-sm text-label-sm text-on-surface-variant" for="partner-code">
              邀請碼
            </label>
            <input id="partner-code"
              ...
              :value="inviteCodeInput" placeholder="輸入邀請碼" type="text" autocomplete="off"
          ...
          <button
            ...
            type="button" :disabled="!canJoinInvite" @click="handleJoinInvite">
        ...
        <button
          ...
          type="button" @click="handleSkipToHome">
