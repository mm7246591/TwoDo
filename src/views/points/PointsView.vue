...
const canUsePoints = computed(() => Boolean(userStore.profile?.uid && userStore.profile?.coupleId))
  ...
  <MobileAppShell>
    <header class="space-y-[20px] px-[20px] pb-[24px] pt-[32px] sm:px-[28px] sm:pt-[40px]">
      ...
      <p class="app-text-muted max-w-[34ch] text-[14px] leading-[24px]">
        這一版把 `task confirmed -> users.points + pointLogs` 接起來，先驗證積分主流程有正確落到 Firestore。
      </p>
    </header>
    ...
    <section class="flex-1 space-y-[16px] px-[20px] pb-[24px] sm:px-[28px]">
      ...
      <section v-else class="grid grid-cols-2 gap-[16px]">
        <article class="app-card px-[16px] py-[16px]">
          ...
          <p class="app-text-strong mt-[8px] text-[30px] font-semibold">{{ currentPoints }}</p>
        </article>

        <article class="app-card-muted px-[16px] py-[16px]">
          <p class="app-label">累積獲得</p>
          <p class="app-text-strong mt-[8px] text-[30px] font-semibold">{{ pointsStore.getEarnedPoints }}</p>
        </article>
      </section>
      ...
      <section class="app-card px-[20px] py-[20px]">
        ...
        <div class="mt-[20px] space-y-[16px]">
          ...
          <p v-if="!pointsStore.pointLogs.length" class="app-text-muted text-[14px] leading-[24px]">
            還沒有積分流水。先去任務頁完成一筆待辦並由建立者確認完成，這裡就會出現第一筆紀錄。
          </p>
        </div>
      </section>

      <p v-if="pointsStore.errorMessage" class="app-banner-danger app-text-danger px-[16px] py-[12px] text-[14px]">
