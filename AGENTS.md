...
- 一律使用繁體中文進行回答。
- 生成skill時請轉為中文
- 使用 skill 產出或更新放在 `.agents/` 內的文件時，需同步複製或更新一份完整副本到對應的 `.claude/` 路徑。若來源是 `.agents/skills/{skill-name}/` 資料夾，目標也必須是 `.claude/skills/{skill-name}/` 資料夾，並保留 `SKILL.md`、references、scripts、assets 等子檔案與子資料夾；不得只建立寫有相對路徑的參照檔。
